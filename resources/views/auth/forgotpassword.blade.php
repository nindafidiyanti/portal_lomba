    ...
    <style>
        ...
        *::after {
        ...
        .bg-grid {
            position: fixed;
            inset: 0;
            background-image: repeating-linear-gradient(90deg, rgba(255, 255, 255, .02) 0px, rgba(255, 255, 255, .02) 1px, transparent 1px, transparent 60px), repeating-linear-gradient(0deg, rgba(255, 255, 255, .02) 0px, rgba(255, 255, 255, .02) 1px, transparent 1px, transparent 60px);
            pointer-events: none;
        }
        ...
        .login-logo {
            ...
            width: 64px;
            height: 64px;
            background: linear-gradient(135deg, var(--accent), #1a5cd6);
            border-radius: 16px;
            margin-bottom: 20px;
    ...
    <div class="login-wrapper">
        <div class="login-card">
            ...
            <form method="POST" action="{{ route('password.email') }}">
                @csrf
                <input type="hidden" name="from" value="{{ request('from', 'user') }}">

                <div class="form-group">
                    <label class="form-label">Email</label>
                    <div class="input-wrap">
                        <input type="email" name="email" class="form-input" placeholder="Masukkan email terdaftar"
            ...
            <div class="login-footer">
                <a href="{{ request('from', old('from', 'user')) === 'admin' ? route('login') : route('login.user') }}">
